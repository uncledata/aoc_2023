...
lines = lines[2:] + ['']
map_list = ['seed-to-soil map:', 'soil-to-fertilizer map:', 'fertilizer-to-water map:', 'water-to-light map:','light-to-temperature map:','temperature-to-humidity map:','humidity-to-location map:'] 
vals = []
...
def iterated_seeds(seed):
    ...
    for mp in map_list:
        relevant_mp = maps[mp]
        if not (relevant_mp[0]["src"][0]>next_val or relevant_mp[-1]["src"][1]<=next_val):
