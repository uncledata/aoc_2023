class Hand:
    ...
    def get_card_str(self):
        d = sorted({char:self.hand.count(char) for char in self.hand}.items(), key=lambda x:x[1], reverse=True)
        #part2
        if 'J' in self.hand:
            if d[0][0]!="J":
                ...
                temp_hand=self.hand
            d = sorted({char:temp_hand.count(char) for char in temp_hand}.items(), key=lambda x:x[1], reverse=True)
        self.strength = 99
        if len(d)==1:
            self.strength = 1
        elif len(d)==2:
